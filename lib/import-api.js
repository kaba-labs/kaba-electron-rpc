...
module.exports = function (channelName, manifest, opts) {
  ...
  var createAPIMethod = {
    sync: (methodName) => {
      return (...args) => {
        ...
        // handle response
        if (typeof success !== "undefined") return IPCValueToValue(success);
    ...
    async: (methodName) => {
      return (...args) => {
        ...
        if (opts.timeout)
          asyncCbTimeouts[requestId] = setTimeout(
            onTimeout,
            opts.timeout,
            requestId
          );
  ...
  function createReadable(requestId) {
    ...
    r.close = (...args) =>
      channel.send("stream-request-close", requestId, ...args);
    return r;
  }
  ...
  function createDuplex(requestId) {
    ...
    stream.end = (...args) =>
      channel.send("stream-request-end", requestId, ...args);
    stream.close = (...args) =>
      channel.send("stream-request-close", requestId, ...args);
    return stream;
  }
  ...
  channel.onMessage(function onIPCMessage(event, msgType, requestId, ...args) {
    console.info(event, msgType, requestId, args);
    ...
    if (msgType.startsWith("stream-")) {
      ...
      if (!stream)
        return api.emit(
          "error",
          new Error(
            "Stream message came from main process for a nonexistant stream"
          ),
          arguments
        );
      ...
      // Event: 'drain'
      if (msgType == "stream-drain") return stream.emit("drain");

      // Event: 'close'
      if (msgType == "stream-close") return stream.emit("close");
      ...
      if (["stream-error", "stream-end", "stream-finish"].includes(msgType)) {
        ...
        if (msgType == "stream-error")
          stream.emit(
            "error",
            fixErrorStack(createError(args[0]), streamErrorStacks[requestId])
          );
        if (msgType == "stream-end") stream.emit("end");
        if (msgType == "stream-finish") stream.emit("finish");
    ...
    if (!requestId && !msgType.startsWith("stream-")) {
      msgType = "async-reply";
      requestId = 0;
      args.push(msgType);
      return onCbReply(requestId, args.map(IPCValueToValue));
  ...
  function onCbReply(requestId, args) {
    ...
    if (asyncCbTimeouts[requestId]) clearTimeout(asyncCbTimeouts[requestId]);
  ...
  function createError(error) {
    ...
    if (!copy)
      copy = new Error(typeof error === "string" ? error : error.message);
